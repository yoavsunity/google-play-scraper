...
const { labelType } = require('../constants');
...
function extractTags (tags) {
  ...
  while (tags.length > 0) {
    ...
    const label = R.path([0], tag);
    const category = R.path([2], tag);
    if (R.is(String, label)) {
      arr.push({
        label,
        url: R.path([1, 4, 2], tag),
        type: category ? labelType.CATEGORY : labelType.SEARCH
      });
    } else {
